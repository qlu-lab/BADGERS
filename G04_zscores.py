#! /z/Comp/lu_group/Members/dyan/Python-2.7.14 python
__author__ = 'Donghui'
import gaact
...
from gaact.gaact_help import AssociationCalculation
from gaact.gaact_help import Utilities as GaactUtilities
from gaact.PredictionModel import  WDBQF


def run(args, _gwas=None):
    start = timer()
    logging.info("Started GAACT association")
    context = GaactUtilities.build_context(args, _gwas)
    ...
    results = GaactUtilities.format_output(results, context, args.remove_ens_version)
    results.to_csv(args.output_file, index=False, header = False, mode = "a+",quoting=csv.QUOTE_NONNUMERIC)
    end = timer()
    logging.info("Sucessfully processed GAACT association in %s seconds"%(str(end - start)))
