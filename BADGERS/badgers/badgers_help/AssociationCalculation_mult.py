...
class ARF(object):
    ...
    order=[(TRAIT,K_TRAIT), (ZSCORE,K_ZSCORE), (EFFECT_SIZE,K_EFFECT_SIZE), (SIGMA_G_2, K_VAR_G), (N_SNPS_IN_MODEL, K_N_SNPS_IN_MODEL), (N_SNPS_IN_COV, K_N_SNPS_IN_COV), (N_SNPS_USED,K_N_SNPS_USED)]
...
def association(trait, context, return_snps=False):
    #capture context  
    n_snps_in_model, i, cov, snps,var,location = context.provide_calculation(trait)
    ...
    if n_snps_used > 0:
        ...
        if sigma_g_2 >0:
            try:
                part_zscore = numpy.sum(i_weight * i_zscore * i_sigma_l)
                part_effect = numpy.sum(i_weight * i_beta * (i_sigma_l**2))
            except Exception as e:
                logging.log(9, "Unexpected exception when calculating zscore: %s, %s", trait, str(e))
    r = (trait, zscore, effect_size, sigma_g_2, n_snps_in_model, 0, n_snps_used)  
    if return_snps:
